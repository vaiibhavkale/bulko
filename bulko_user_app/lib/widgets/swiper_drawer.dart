...
class SwiperDrawer extends StatefulWidget {
  ...
  SwiperDrawerState createState() => SwiperDrawerState(
      ...
      bodyBackgroundPeekSize: bodyBackgroundPeekSize,
      bodySize: bodySize,
      curve: curve,
      hasClone: hasClone,
      radius: radius,
...
class SwiperDrawerState extends State<SwiperDrawer>
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
        ...
        builder: (context, snapshot) {
          double scale = 1.3 + -0.3 * (1 - animation.value);
              ...
              if (hasClone)
                Opacity(
                  ...
                  child: buildAnimationBody(scaleSmall, size,
                      bodySize + bodyBackgroundPeekSize, reverse),
                ),
              GestureDetector(
                  onHorizontalDragStart: (details) {},
                  behavior: HitTestBehavior.translucent,
                  child: buildAnimationBody(scale, size, bodySize, reverse)),
              Transform.translate(
                offset:
  ...
  Container buildDrawer(Size size) {
    return Container(width: size.width, child: drawer);
  }
