...
class _PaymentGatewayScreenState extends BaseRouteState {
                  ...
                  children: [
                    ...
                    screenId! > 1
                      ...
                      onTap: () async {
                        ...
                        //       content: Text('Are you sure you want to use Cash on Delivery (COD)?'),
                        //       actions: <Widget>[
                        //         TextButton(
                        //           child: Text('Cancel'),
                        //           onPressed: () {
  ...
  //         } else {
  //           hideLoader();
  //           showSnackBar(key: _scaffoldKey, snackBarMessage: 'Something went wro ${result.message}');
  //         }
  //       });
  //     } else {
  ...
  //               showSnackBar(key: _scaffoldKey, snackBarMessage: '${result['message']}');
  //             } else {
  //               showSnackBar(key: _scaffoldKey, snackBarMessage: 'Unknown error occurred');
  //             }
  //           }
  //         } else {
  //           hideLoader();
  ...
  //     }
  //   } catch (e) {
  //     print("Exception occurred in _orderCheckOut(): $e");
  //     // Print more specific error message based on the type of exception
  //     if (e is DioError) {
      ...
      if (isConnected) {
        await apiHelper.checkout(cartId: order!.cartid, paymentStatus: paymentStatus, paymentMethod: paymentMethod, wallet: _isWallet == 1 ? 'yes' : 'no', paymentId: paymentId, paymentGateway: paymentGateway).then((result) async {
          ...
          if (result != null && result['status'] != null && result['status'] == "1") {
            ...
            showDialog(
                ...
                builder: (BuildContext context) {
                  return AlertDialog(
                    ...
                    title: Text(
                      "${AppLocalizations.of(context)!.tle_order_placed}",
                    ),
                    content: Image.asset(
            ...
            //print("API Response: $result"); // Print the entire result object
            //String errorMessage = result != null && result['message'] != null ? result['message'] : 'Unknown error occurred';
            //print("Error Message: $errorMessage"); // Print the specific message field
            //showSnackBar(key: _scaffoldKey, snackBarMessage: errorMessage); // Show the error message in the snackbar
            // Show the error message in the snackbar
            // String errorMessage = result != null && result['message'] != null ? result['message'] : 'Unknown error occurred';
            // showSnackBar(key: _scaffoldKey, snackBarMessage: errorMessage);
          }
        });
      } else {
        showNetworkErrorSnackBar(_scaffoldKey);
